...
function atualizarAlertas(temperatura, umidade, temps, umids) {
    ...
    if (umidade >= 80 || umidade <= 25) alertas.push({ text: `Umidade ${umidade >= 80 ? "muito alta" : "muito baixa"}: ${umidade.toFixed(0)}%`, type: "warning" });
...
function montarHistorico(labels, temps, umids) {
    ...
    historicoGrafico = new Chart(historyCanvas, {
        ...
        options: {
            ...
            scales: {
                x: { ticks: { color: "rgba(223, 239, 255, 0.6)", maxTicksLimit: 5 }, grid: { display: false }, border: { display: false } },
                tempHistory: { position: "left", ticks: { color: "rgba(238, 155, 136, 0.8)", maxTicksLimit: 4 }, grid: { color: "rgba(191, 221, 245, 0.08)" }, border: { display: false } },
                humidityHistory: { position: "right", ticks: { color: "rgba(139, 199, 220, 0.8)", maxTicksLimit: 4 }, grid: { display: false }, border: { display: false } }
            }
        }
    });
}
...
function montarGrafico(labels, temps, umids) {
    ...
    const gridColor = "rgba(191, 221, 245, 0.12)";
    const textColor = "rgba(223, 239, 255, 0.7)";
    ...
    grafico = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    backgroundColor: "rgba(8, 18, 30, 0.94)",
                    titleColor: "#f5fbff",
                    bodyColor: "#dfefff",
...
techFormEl.addEventListener("submit", (event) => {
    ...
    const pergunta = techInputEl.value.trim();
    if (!pergunta) return;
    adicionarMensagem(pergunta, "user");
    techInputEl.value = "";
    window.setTimeout(() => {
        const resposta = responderTech(pergunta);
